...
var game;
(function (game) {
    var EquipSpecialView = (function (_super) {
        __extends(EquipSpecialView, _super);
        function EquipSpecialView(viewConf) {
            ...
            _this._starArray = [];
            _this._enough = false;
            _this._ringActiveEffs = [];
            return _this;
        }
        EquipSpecialView.prototype.childrenCreated = function () {
            var _this = this;
            ...
            this.lb_ring_fragment0.addEventListener(egret.TouchEvent.TOUCH_TAP, function () { _this.activeRingFragment(1); }, this);
            this.lb_ring_fragment1.addEventListener(egret.TouchEvent.TOUCH_TAP, function () { _this.activeRingFragment(2); }, this);
            this.lb_ring_fragment2.addEventListener(egret.TouchEvent.TOUCH_TAP, function () { _this.activeRingFragment(3); }, this);
            this.lb_ring_fragment3.addEventListener(egret.TouchEvent.TOUCH_TAP, function () { _this.activeRingFragment(4); }, this);
            this.btn_other_upgrade.addEventListener(egret.TouchEvent.TOUCH_TAP, this.upgradeEquip, this);
            this.initView();
        };
        EquipSpecialView.prototype.initView = function () {
        ...
        EquipSpecialView.prototype.addActiveEff = function () {
            ...
            for (var i = 0; i < this._ringActiveEffs.length; i++) {
        ...
        EquipSpecialView.prototype.clearActiveEff = function () {
            for (var i = 0; i < this._ringActiveEffs.length; i++) {
        ...
        EquipSpecialView.prototype.updateView = function (data) {
            var _this = this;
            if (data || data == 0) {
                this._curPos = data;
            }
            if (this._curPart == ConstSpecialEquipPart.PARA_RING || this._curPart == ConstSpecialEquipPart.BODY_RING) {
                RES.getResAsync("equip_sp_shenjiejinjie_title_png", function (texture) {
                    _this.commonWin.img_title.texture = texture;
                }, this);
            }
            else {
                RES.getResAsync("equip_sp_zhenxizhuangbeishengji_title_png", function (texture) {
                    _this.commonWin.img_title.texture = texture;
                }, this);
            }
            //特效
            this._mcEffect.playMCKey(ConstSpecialEquipEffect[this._curPart], "", -1, null, function () {
                _this._mcEffect.frameRate = 8;
            }, null, this);
            var specialInfo = this.heroModel.heroInfo[this._curPos].getEquipSpecialByPart(this._curPart);
            if (specialInfo.id) {
                var info = App.ConfigManager.getEquipSpecialById(specialInfo.id);
                if (this._curPart == ConstSpecialEquipPart.PARA_RING || this._curPart == ConstSpecialEquipPart.BODY_RING) {
                ...
                this.gp_other.visible = false;
                this.gp_ring.visible = false;
                var info = App.ConfigManager.getEquipSpecialByPartLevel(this._curPart, 0);
                this.lb_active_desc.text = info.des;
                if (info.condition && typeof info.condition == "number") {
                    var sceneInfo = App.ConfigManager.getHookSceneConfigByLevel(info.condition);
        ...
        //戒指
        EquipSpecialView.prototype.updateRingView = function () {
            var specialInfo = this.heroModel.heroInfo[this._curPos].getEquipSpecialByPart(this._curPart);
            var ringInfo = App.ConfigManager.getEquipSpecialById(specialInfo.id);
            var nextInfo = App.ConfigManager.getEquipSpecialById(ringInfo.next_id);
            this.lb_name.text = ringInfo.name;
            if (0 && ringInfo.next_id) {
                for (var i = 0; i < 4; i++) {
                    var singleRingInfo = App.ConfigManager.getEquipSpecialFragmentById(nextInfo.ring_id[i]);
                    this._ringFragment[i].text = singleRingInfo.name;
                    if (specialInfo.getpieceByKey(ringInfo.ring_id[i])) {
                    ...
                    else if (game.BossModel.getInstance().level >= singleRingInfo.condition) {
                ...
                if (ringInfo.next_id) {
                    ...
                    for (var i = 0; i < 4; i++) {
                        var singleRingInfo = App.ConfigManager.getEquipSpecialFragmentById(ringInfo.ring_id[i]);
                        this._ringFragment[i].text = singleRingInfo.name;
                        if (specialInfo.getpieceByKey(ringInfo.ring_id[i])) {
                            this._ringActiveEffs[i].visible = false;
                            this._ringActiveEffs[i].stop();
            ...
            //属性
            var attribute = App.ConfigManager.getAttributeInfoById(ringInfo.add_id);
            ...
            for (var key in attrBase) {
                if (this.heroModel.heroInfo[this._curPos].job == CareerType.SOLDIER) {
                    if (key != "mac" && key != "sc") {
                        textArray.push({ text: ConstAttribute[key] + ": ", style: { textColor: 0xc6be66 } }, { text: attrBase[key], style: { textColor: 0xc6bebb } }, { text: "\n" });
                    }
                }
            ...
            if (ringInfo.next_id) {
                ...
                var attribute2 = App.ConfigManager.getAttributeInfoById(nextInfo.add_id);
                var attrBase2 = game.EquipModel.getInstance().attributeFilter(attribute2);
                var textArray2 = [];
                for (var key in attrBase2) {
                    if (this.heroModel.heroInfo[this._curPos].job == CareerType.SOLDIER) {
                        if (key != "mac" && key != "sc") {
                            textArray2.push({ text: ConstAttribute[key] + ": ", style: { textColor: 0xbfbfbf } }, { text: attrBase2[key], style: { textColor: 0xc6bebb } }, { text: "\n" });
                        }
                    }
                    else if (this.heroModel.heroInfo[this._curPos].job == CareerType.MAGES) {
                        if (key != "ac" && key != "sc") {
        ...
        //其他
        EquipSpecialView.prototype.updateOtherView = function () {
            var _this = this;
            var specialInfo = this.heroModel.heroInfo[this._curPos].getEquipSpecialByPart(this._curPart);
            var otherInfo = App.ConfigManager.getEquipSpecialById(specialInfo.id);
            var afterLevel = otherInfo.level % 10; //激活星星数
            this.lb_name.text = otherInfo.name;
            if (afterLevel == 0 && otherInfo.level != 0) {
                afterLevel = 10;
            }
            for (var i = 0; i < this._starArray.length; i++) {
                if (i + 1 <= afterLevel) {
            ...
            if (otherInfo.next_id) {
                var nextInfo = App.ConfigManager.getEquipSpecialById(otherInfo.next_id);
                ...
                if (itemInfo.num >= cost[2]) {
                    ...
                    RES.getResAsync("equip_sp_txt_shengji_png", function (texture) {
                        _this.img_other_txt.texture = texture;
                    }, this);
                }
                else {
                    this._enough = false;
                    ...
                    RES.getResAsync("equip_sp_txt_huoqusuipian_png", function (texture) {
                        _this.img_other_txt.texture = texture;
                    }, this);
                }
            }
            ...
            //属性
            var attribute = App.ConfigManager.getAttributeInfoById(otherInfo.add_id);
            var attrBase = game.EquipModel.getInstance().attributeFilter(attribute);
            var textArray = [];
            for (var key in attrBase) {
                if (this.heroModel.heroInfo[this._curPos].job == CareerType.SOLDIER) {
                    if (key != "mac" && key != "sc") {
                        textArray.push({ text: ConstAttribute[key] + ": ", style: { textColor: 0xbfbfbf } }, { text: attrBase[key], style: { textColor: 0x00f829 } }, { text: "\n" });
                    }
                }
                else if (this.heroModel.heroInfo[this._curPos].job == CareerType.MAGES) {
                    if (key != "ac" && key != "sc") {
            ...
            if (otherInfo.next_id) {
                var nextInfo2 = App.ConfigManager.getEquipSpecialById(otherInfo.next_id);
                var attribute2 = App.ConfigManager.getAttributeInfoById(nextInfo2.add_id);
                var attrBase2 = game.EquipModel.getInstance().attributeFilter(attribute2);
                var textArray2 = [];
                for (var key in attrBase2) {
                    if (this.heroModel.heroInfo[this._curPos].job == CareerType.SOLDIER) {
                        if (key != "mac" && key != "sc") {
                            textArray2.push({ text: ConstAttribute[key] + ": ", style: { textColor: 0xc6be66 } }, { text: attrBase2[key], style: { textColor: 0x00f829 } }, { text: "\n" });
                        }
                    }
                    else if (this.heroModel.heroInfo[this._curPos].job == CareerType.MAGES) {
                        if (key != "ac" && key != "sc") {
        ...
        //特殊装备升级 
        EquipSpecialView.prototype.upgradeEquip = function () {
            if (this._enough) {
        ...
        EquipSpecialView.prototype.checkGuide = function () {
        ...
        EquipSpecialView.prototype.removeGuide = function () {
         ...
         *
         */
        EquipSpecialView.prototype.openWin = function (openParam) {
         ...
         * 关闭窗口
         */
        EquipSpecialView.prototype.closeWin = function (callback) {
         ...
         * 清理
         */
        EquipSpecialView.prototype.clear = function (data) {
         ...
         * 销毁
         */
        EquipSpecialView.prototype.destroy = function () {
            _super.prototype.destroy.call(this);
            this._mcEffect.destroy();
        };
        return EquipSpecialView;
    }(BaseView));
    game.EquipSpecialView = EquipSpecialView;
    __reflect(EquipSpecialView.prototype, "game.EquipSpecialView");
})(game || (game = {}));
//# sourceMappingURL=EquipSpecialView.js.map