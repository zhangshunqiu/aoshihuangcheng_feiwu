...
class BaseConfManager {
	...
	public getConfig(name:string):any{
		if (this._confDic[name]==null) {
			this._confDic[name] = RES.getRes(name);
		}
...
class BaseSound {
    ...
    public constructor() {
        ...
        TimerManager.getInstance().doTimer(1 * 60 * 1000, 0, this.dealSoundTimer, this);
    }
...
class BaseView extends eui.Component {
	...
	public constructor(viewConf: WinManagerVO = null) {
		...
		if (this.winVo && this.winVo.param && this.winVo.param.y) {
	...
	private openWinAnimation() {
		if (this.winVo && this.winVo.openModel != 0) {
			...
			if (true) {
				...
				darkSprite.graphics.beginFill(0x000000, 0.8);
				...
				darkSprite.x = (this.width - darkSprite.width)/2;
				darkSprite.y = (this.height - darkSprite.height)/2;
				if (!this.contains(darkSprite)) {
					this.addChild(darkSprite);
					this.setChildIndex(darkSprite, 0);
...
class GlobalTimer {
	...
	public addSchedule(delay: number, repeatCount: number, method: Function, methodObj: any, complateMethod: Function = null, complateMethodObj: any = null):number{
        ...
        vo.complateMethod = complateMethod;
        vo.complateMethodObj = complateMethodObj;
		...
		if(repeatCount != 0 && complateMethod && complateMethodObj){
			cTimer.addEventListener(egret.TimerEvent.TIMER_COMPLETE, complateMethod, complateMethodObj);
		}
		cTimer.start()
		vo.curTimer = cTimer;
		...
		if(this._hashObject[timerId]){
			...
			if(vo.tType == 1){
				if(vo.curTimer){
					...
					if(vo.complateMethod && vo.complateMethodObj){
						vo.curTimer.removeEventListener(egret.TimerEvent.TIMER_COMPLETE, vo.complateMethod, vo.complateMethodObj);
					}
					vo.curTimer.stop()
				}
			}else if(vo.tType == 2){
    ...
    private onEnterFrame():void {
        ...
        for (var i:number = 0; i < this._count; i++) {
            ...
            if (t >= handler.exeTime) {
                // console.log(handler.method.toString());
